...
export default function ChatGroq() {
  ...
  const handleStreamCall = useCallback(
    async (promptText, imageData = null) => {
      if (!promptText?.trim() && !imageData) return;
      ...
      try {
        ...
        setMessages((prev) => {
          ...
          return [...prev, { from: "bot", text: "🤔 Analyse en cours..." }];
        });
  
        // 🧠 Si on a une image => appel à Groq Vision
        if (imageData) {
          ...
          const res = await fetch(`${API_URL}/api/groq-vision`, {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              prompt: promptText || "Décris cette image avec précision",
              imageBase64: base64,
            }),
          });
  
          const data = await res.json();
          fullText = data?.choices?.[0]?.message?.content || "❌ Aucune réponse reçue.";
        } 
        // 🧠 Sinon, message texte classique via ton modèle texte (DeepSeek)
        else {
          ...
          const data = await response.json();
          fullText = data.message?.content || data?.choices?.[0]?.message?.content || "❌ Aucune réponse.";
        }
